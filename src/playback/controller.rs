...
use super::player::{self, ControlCmd, Input, SharedReader};
...
impl PlayerController {
    pub fn new(event_tx: tokio_mpsc::UnboundedSender<Event>) -> Self {
        ...
        std::thread::spawn(move || {
                ...
                .build()
            {
                Ok(rt) => rt,
                Err(e) => {
                    log::error!("Failed to create player runtime: {e}");
                    return;
                }
            };
    ...
    pub fn request(&self, input: Input, seek_time: Option<Duration>) {
        if self
            .cmd_tx
            .send(PlayerCmd::Play { input, seek_time })
            .is_err()
        {
            log::error!("PlayerController: failed to send Play command (channel closed)");
        }
    }

    pub fn pause(&self) {
        if self.cmd_tx.send(PlayerCmd::Pause).is_err() {
            log::error!("PlayerController: failed to send Pause command (channel closed)");
        }
    }

    pub fn resume(&self) {
        if self.cmd_tx.send(PlayerCmd::Resume).is_err() {
            log::error!("PlayerController: failed to send Resume command (channel closed)");
        }
    }

    pub fn seek_to(&self, seek_time: Duration) {
        if self.cmd_tx.send(PlayerCmd::SeekTo(seek_time)).is_err() {
            log::error!("PlayerController: failed to send SeekTo command (channel closed)");
        }
    }

    pub fn stop(&self) {
        if self.cmd_tx.send(PlayerCmd::Stop).is_err() {
            log::error!("PlayerController: failed to send Stop command (channel closed)");
        }
    }

    pub fn set_volume(&self, volume: f32) {
        if self.cmd_tx.send(PlayerCmd::SetVolume(volume)).is_err() {
            log::error!("PlayerController: failed to send SetVolume command (channel closed)");
        }
    }
}
