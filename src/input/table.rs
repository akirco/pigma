...
pub(super) fn toggle_table_mode(app: &mut App) {
    ...
    match nav.table_mode {
        TableMode::Row => {
            ...
            let columns = app.config.columns.for_content(nav.content.content_type(), None).to_vec();
...
pub(super) fn cell_select_prev_column(app: &mut App) {
    let nav = &mut app.state.navigation;
    let columns = app.config.columns.for_content(nav.content.content_type(), None);
    let n = columns.len();
    if n == 0 {
        return;
    }
